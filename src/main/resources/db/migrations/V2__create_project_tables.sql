...
CREATE TABLE land_details (
    ...
    updated_by VARCHAR(50),
    updated_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_landdetails_project FOREIGN KEY (project_id) REFERENCES project(id) ON DELETE CASCADE
);
...
CREATE TABLE blueprint_details (
    ...
    type VARCHAR(50),
    nationalized_bank_load BOOLEAN DEFAULT FALSE,
    proposal_code VARCHAR(50),
    created_by VARCHAR(50),
    created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_by VARCHAR(50),
    updated_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_blueprintdetails_project FOREIGN KEY (project_id) REFERENCES project(id) ON DELETE CASCADE
);
